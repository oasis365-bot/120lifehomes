...
test('runOnce: requestBody를 넘기면 기본값({}) 대신 그 값을 그대로 보낸다', async () => {
  let seenBody;
  const fetchImpl = fakeFetch(async (url, opt) => {
    seenBody = opt.body;
    return { ok: true, status: 200, json: async () => ({ ok: true, status: 'item_complete', phase: 'enrichment', didWork: true }) };
  });
  await runOnce({
    baseUrl: 'https://x.invalid', cronSecret: SECRET, fetchImpl,
    requestBody: JSON.stringify({ singleItemTest: true }),
  });
  assert.equal(seenBody, '{"singleItemTest":true}');
});
...
for (const [name, value] of [
  ...
  test(`isSingleItemTestRequested: ${name}이면 시험 요청 아님으로 취급한다`, () => {
    const env = value === undefined ? {} : { HOSPITAL_BATCH_SINGLE_ITEM_TEST: value };
    assert.equal(isSingleItemTestRequested(env), false);
  });
}

test("isSingleItemTestRequested: 정확히 문자열 '1'일 때만 시험 요청으로 취급한다", () => {
  assert.equal(isSingleItemTestRequested({ HOSPITAL_BATCH_SINGLE_ITEM_TEST: '1' }), true);
});

test('main(): [수동 시험 경로] 예약 스위치가 꺼져 있어도(그 값을 바꾸지도 않고) single-item 시험은 진행되며, singleItemTest:true 바디로 정확히 1회 호출한다', async () => {
  const originalFetch = globalThis.fetch;
  const originalLog = console.log;
  const logs = [];
  // 저장소 변수는 명시적으로 OFF로 둔 채 시작한다 — 이 테스트의 핵심은
  // "시험 경로가 이 값을 절대 바꾸지 않는다"는 것이다.
  delete process.env.HOSPITAL_BATCH_SCHEDULE_ENABLED;
  process.env.HOSPITAL_BATCH_SINGLE_ITEM_TEST = '1';
  process.env.HOSPITAL_BATCH_BASE_URL = 'https://x.invalid';
  process.env.HOSPITAL_BATCH_CRON_SECRET = SECRET;
  let calls = 0; let seenBody;
  globalThis.fetch = async (url, opt) => {
    calls += 1; seenBody = opt.body;
    return {
      ok: true, status: 200,
      json: async () => ({ ok: true, status: 'item_complete', phase: 'enrichment', didWork: true, attempted: 1, completed: 1, retried: 0, deadLettered: 0 }),
    };
  ...
  try {
    ...
    globalThis.fetch = originalFetch;
    console.log = originalLog;
    delete process.env.HOSPITAL_BATCH_SINGLE_ITEM_TEST;
    delete process.env.HOSPITAL_BATCH_BASE_URL;
    delete process.env.HOSPITAL_BATCH_CRON_SECRET;
  }

  assert.equal(calls, 1, '시험은 정확히 1번만 API를 호출해야 함');
  assert.equal(seenBody, '{"singleItemTest":true}', '서버가 개수를 강제할 수 있도록 singleItemTest:true를 보내야 함');
  assert.equal(process.env.HOSPITAL_BATCH_SCHEDULE_ENABLED, undefined, '시험 경로가 예약 스위치 값을 건드리면 안 됨');
  assert.equal(process.exitCode, 0);
  process.exitCode = 0;
  assert.match(logs.join('\n'), /single-item test requested/);
});

test('main(): [예약 실행 경로] single_item_test 입력이 없으면(정상 스케줄/수동) 기존 스위치만 그대로 따르고 일반 바디({})로 호출한다', async () => {
  const originalFetch = globalThis.fetch;
  const originalLog = console.log;
  delete process.env.HOSPITAL_BATCH_SINGLE_ITEM_TEST; // 시험 입력 없음 = 예약 경로
  process.env.HOSPITAL_BATCH_SCHEDULE_ENABLED = '1';
  process.env.HOSPITAL_BATCH_BASE_URL = 'https://x.invalid';
  process.env.HOSPITAL_BATCH_CRON_SECRET = SECRET;
  let calls = 0; let seenBody;
  ...
  assert.equal(seenBody, '{}', '시험 경로가 아니면 singleItemTest 바디를 보내면 안 됨(개수 제한 없는 정상 처리)');
  assert.equal(process.exitCode, 0);
  process.exitCode = 0;
});

test('main(): [예약 실행 경로] single_item_test가 없고 예약 스위치도 꺼져 있으면 시험 경로와 무관하게 그대로 스킵한다(회귀)', async () => {
  const originalFetch = globalThis.fetch;
  const originalLog = console.log;
  delete process.env.HOSPITAL_BATCH_SINGLE_ITEM_TEST;
  delete process.env.HOSPITAL_BATCH_SCHEDULE_ENABLED;
