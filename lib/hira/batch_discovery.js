...
export async function runDiscoveryPage(deps = {}) {
  ...
  try {
    ...
    if (sourceRows.length) {
      await storageStep('sources_write', () => sb('facility_sources?on_conflict=source_system,external_id', {
        method: 'POST', body: sourceRows, prefer: 'resolution=merge-duplicates,return=minimal',
      }));
    }
    if (rows.length) {
      await storageStep('items_write', () => sb('hospital_collection_items?on_conflict=job_id,facility_id', {
