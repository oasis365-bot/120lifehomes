...
// Supabase/PostgREST 프로젝트의 Max Rows 설정이 요청한 limit보다 낮으면 응답이
// 조용히 더 작게 잘린다(예: limit=2000 요청해도 최대 1000개만 옴). item 수가 그
// 상한을 넘으면 seen/ordinal 계산이 최근 항목을 놓쳐, 이미 쓴 ordinal과 충돌해
// 새 항목이 on conflict do nothing으로 조용히 버려지는 문제가 있었다. 실제 반환된
// 길이만큼만 offset을 전진시키고 빈 페이지가 나올 때까지 반복해 상한과 무관하게
// 전체를 읽는다.
async function fetchAllExistingItems(sb, jobId) {
  const items = [];
  let offset = 0;
  for (let guard = 0; guard < 50; guard += 1) {
    const batch = (await sb(
      `hospital_collection_items?job_id=eq.${enc(jobId)}` +
      `&select=facility_id,ordinal&order=ordinal.asc&limit=1000&offset=${offset}`
    )).data || [];
...
export async function runDiscoveryPage(deps = {}) {
  ...
  try {
    const existing = await storageStep('items_read', () => fetchAllExistingItems(sb, job.id));
