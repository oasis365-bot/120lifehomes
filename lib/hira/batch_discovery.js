...
function discoveryErrorCode(error, { quota, upstreamLimit, retryable }) {
  ...
  if (hira) {
    ...
    if (hira.reason === 'result') {
      // TEMP DIAGNOSTIC (fix/discovery-count-mismatch-diag) — 원인 확인 후 원복할 것.
      const diagMatch = /(diag_tc\d+_sn\d+_p\d+_nr\d+)/.exec(String(error?.message || ''));
      if (diagMatch) return diagMatch[1];
      return BATCH_ERROR_CODES.hiraResponseInvalid;
    }
...
export async function runDiscoveryPage(deps = {}) {
  ...
  try {
    ...
    if (complete && seen.size !== page.totalCount) {
      // TEMP DIAGNOSTIC (fix/discovery-count-mismatch-diag) — 집계 숫자만 메시지에 실어
      // last_error_code로 노출한다(기관 식별정보 없음). 원인 확인 후 원복할 것.
      throw new HiraError(
        `HIRA snapshot 수량 불일치 diag_tc${page.totalCount}_sn${seen.size}_p${pageNo}_nr${rows.length}`,
