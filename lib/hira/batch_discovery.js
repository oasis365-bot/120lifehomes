...
function retryLater(error) {
  return error instanceof HiraError && RETRY_LATER_FAILURE_KINDS.has(error.failureKind);
}
...
function discoveryErrorCode(error, { quota, upstreamLimit, retryable }) {
  ...
  if (retryable) {
    ...
    return map[error.failureKind] || BATCH_ERROR_CODES.discoveryFailed;
  }
  // 원문·URL·키 없이도 운영자가 다음 조치를 판단할 수 있도록, 재시도 대상이 아닌
  // HIRA 오류는 안전한 범주 코드로만 구분한다.
  if (error instanceof HiraError) {
    if (error.reason === 'gateway') return BATCH_ERROR_CODES.hiraGatewayError;
    if (error.reason === 'result') return BATCH_ERROR_CODES.hiraResponseInvalid;
    if (error.reason === 'deadline') return BATCH_ERROR_CODES.hiraDeadlineExceeded;
    if (error.reason === 'quota_guard') return BATCH_ERROR_CODES.quotaReservationFailed;
    if (error.reason === 'aborted_all' || error.reason === 'network' || error.reason === 'timeout') {
