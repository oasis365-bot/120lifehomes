...
export const FAILURE_KINDS = Object.freeze([
  ...
  'quota_exhausted', // 애플리케이션 일일 호출 예약 거부 (실제 HTTP 요청 전 차단)
  'quota_reservation_failed', // 일일 호출 예약 저장소/RPC 실패 (한도 소진과 구분)
  'upstream_quota', // HIRA 정상 HTTP 본문의 일일한도 코드(예: resultCode 22)
 ...
 * @param {() => number} [opt.randomImpl]  backoff jitter 용 (기본 Math.random — 테스트 결정론화 시 주입)
 * @param {(ms:number)=>void} [opt.onWait]  (테스트 관찰용)
 * @param {(info:{op:string,attempt:number})=>Promise<boolean>|boolean} [opt.beforeAttempt]
 *   실제 fetch 시도 직전 호출. false면 quota_exhausted, throw면 quota_reservation_failed로 중단.
 */
export function createHiraClient(opt = {}) {
  ...
  const random = typeof opt.randomImpl === 'function' ? opt.randomImpl : Math.random;
  ...
  async function request(base, op, params) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      if (typeof opt.beforeAttempt === 'function') {
        try {
          const allowed = await opt.beforeAttempt({ op, attempt });
          if (allowed === false) {
            throw new HiraError('HIRA 일일 호출 예약 거부', {
              op, reason: 'quota', failureKind: 'quota_exhausted', attempts: attempt - 1,
            });
          }
        } catch (e) {
          if (e instanceof HiraError && e.failureKind === 'quota_exhausted') throw e;
          throw new HiraError('HIRA 일일 호출 예약 실패', {
            op, reason: 'quota_guard', failureKind: 'quota_reservation_failed', attempts: attempt - 1,
          });
        }
      }
      // quota 예약 중에도 wall-clock 예산이 소진될 수 있다. 예약은 fail-safe로
      // 되돌리지 않되, 데드라인이 임박했다면 새 HIRA fetch는 시작하지 않는다.
      if (timeLeft() <= DEADLINE_PAD_MS + 500) { deadlineHit = true; break; }
      ...
      if (r.networkError) {
        ...
        if (attempt < maxRetries) {
          if (await retryBackoff(attempt) === 'deadline') { deadlineHit = true; break; }
          continue;
        }
        break;
      }
  ...
  // backoff = 250·2^(n-1) + jitter(0~299). jitter 는 randomImpl 로 주입 가능(테스트 결정론화).
  const backoffMsFor = (attempt) => 250 * Math.pow(2, attempt - 1) + Math.floor(random() * 300);
  /**
   * 이번 재시도의 backoff 를 (예산 안이면) 수행한다.
   *  · backoff 시간은 한 번만 계산해 판단과 실제 sleep 에 같은 값을 쓴다(불일치 제거).
   *  · 남은 예산이 (backoff + 다음 호출 최소여유 1000ms) 보다 작으면 재시도하지 않고 'deadline'.
   * @returns {'ok'|'deadline'}
   */
  async function retryBackoff(attempt) {
    const bo = backoffMsFor(attempt);
    if (timeLeft() <= bo + DEADLINE_PAD_MS + 500) return 'deadline';
    await sleep(Math.min(bo, Math.max(0, timeLeft() - DEADLINE_PAD_MS)));
    return 'ok';
  }
