...
export function normalizeHospitalRecord(bundle) {
  ...
  normalized.normalized_hash = normalizedHash(stripForHash(normalized));
  return normalized;
}
...
/**
 * normalized_hash 계산 대상 뷰 — 메타(_warnings)와 해시 자신을 제외한 정규화 데이터.
 * collect.js 가 재수집 판정 해시에 endpoint 상태(sources)를 함께 접어 넣을 때 재사용한다.
 * @param {object} n normalizeHospitalRecord 결과
 * @returns {object}
 */
export function stripForHash(n) {
