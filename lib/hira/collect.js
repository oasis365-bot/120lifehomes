...
// 상세 endpoint 별 수집 결과 상태 (persist 의 필드별 PATCH 판단용 내부 메타).
//   success_with_data : resultCode 정상 + 항목 존재
//   success_empty     : resultCode 정상 + 항목 없음 (권위 있는 "없음")
//   failed            : throw(timeout/network/5xx) 또는 비정상 resultCode
//   unavailable       : 시간예산 등으로 호출 자체를 못 함
 ...
 * @param {Array<{ykiho:string,basis:object}>} [opt.seededInstitutions]
 *   discovery snapshot 에 이미 보관한 basis 를 사용해 목록 API를 다시 호출하지 않고
 *   상세·평가만 수집한다. ykiho 는 호출 중 메모리에만 존재하며 반환값에는 노출하지 않는다.
...
export async function collectHospitals(client, opt = {}) {
  ...
  const seededInstitutions = Array.isArray(opt.seededInstitutions) ? opt.seededInstitutions : null;
  ...
  // ── 1) 목록 수집 또는 discovery snapshot 주입 ──
  const seen = new Set();
  const ykihos = [];
  ...
  if (seededInstitutions) {
    for (const entry of seededInstitutions.slice(0, maxInstitutions)) {
      const ykiho = typeof entry?.ykiho === 'string' ? entry.ykiho.trim() : '';
      const basis = entry?.basis;
      if (!ykiho || !basis || typeof basis !== 'object' || Array.isArray(basis) || seen.has(ykiho)) continue;
      seen.add(ykiho);
      ykihos.push({ ykiho, basis });
    }
    stats.listed = ykihos.length;
    stats.deduped = ykihos.length;
    meta.mode = 'snapshot_enrichment';
  } else {
    let pageNo = 1;
    ...
    // eslint-disable-next-line no-await-in-loop
      ...
      warnings.push(`목록 p${pageNo}: 시간 예산 초과로 중단`);
        ...
        throw new HiraError('수집 시간 예산 초과', {
          op: 'getHospBasisList', reason: 'deadline_exceeded', failureKind: 'deadline',
          attempts: r.attempts, lastResultCode: null,
        });
        }
        break;
      }
      if (r.kind === 'error') {
      const ce = r.error || {};
      const clientAttempts = Number.isFinite(ce.attempts) ? ce.attempts : null;
      warnings.push(`목록 p${pageNo} 조회 실패 — ${briefErr(r.error)}`);
        if (ykihos.length === 0) {
        // 첫 페이지부터 목록을 못 받음 → 보존할 부분 결과 없음. 빈 배열로 "성공" 반환 안 함.
        const deadline = ce.reason === 'deadline';
        throw new HiraError(deadline ? '수집 시간 예산 초과' : 'HIRA 목록 조회 실패', {
          op: 'getHospBasisList',
          reason: deadline ? 'deadline_exceeded' : 'list_fetch_failed',
          // client 가 분류한 실패 종류·시도별 집계·경과 구간을 그대로 전달 (allowlist).
          failureKind: deadline ? 'deadline' : (ce.failureKind || 'unknown'),
          attemptSummary: ce.attemptSummary || null,
          elapsedBucket: ce.elapsedBucket || null,
          attempts: clientAttempts ?? r.attempts, // client 내부 재시도 횟수
          lastResultCode: ce.lastResultCode || null,
        });
        }
      ...
      warnings.push(`목록 p${pageNo} 비정상 resultCode`);
        if (ykihos.length === 0) {
        const rc = String((r.parsed && r.parsed.resultCode) || '');
        throw new HiraError('HIRA 목록 비정상 응답', {
          op: 'getHospBasisList',
          reason: 'list_abnormal_result',
          failureKind: rc === '12' ? 'result_code_12' : 'unknown',
          attempts: r.attempts,
          lastResultCode: rc || null,
        });
        }
        break;
      }
      ...
      warnings.push(`목록 p${pageNo}: 정상 resultCode 인데 ${r.attempts}회 모두 0건 — transient_empty_page_exhausted`);
        ...
        failureKind: 'unknown', // resultCode 자체는 정상 — 네트워크/게이트웨이 분류 대상 아님
        attempts: r.attempts,
  ...
  for (const { ykiho, basis } of ykihos) {
    if (listOnly) {
      ...
      // listOnly(readiness) 는 상세 endpoint 를 호출하지 않으므로 sources 는 비어 있다.
      // persist 는 호출되지 않지만(드라이런), 방어적으로 빈 맵을 실어 둔다.
      normalizedAll.push({ hospital: norm, evaluation: null, raw: { basis }, sources: {} });
    ...
    const bundle = { basis };
    let stepFailures = 0;
    // endpoint 별 상태 (persist 의 필드별 PATCH 판단용 내부 메타. 응답·raw·로그 미노출)
    const stepStatus = {};
    ...
    for (const [name, fn] of DETAIL_STEPS) {
      ...
      try {
        ...
        if (!isNormalResult(parsed)) {
          stepFailures += 1;
          stepStatus[name] = SOURCE_STATE.FAILED;
          failures.push({
        ...
        // facility/detail = 단수(첫 item), 나머지 = 배열
        if (name === 'facility' || name === 'detail') {
          const it = firstItem(parsed);
          bundle[name] = it;
          stepStatus[name] = it ? SOURCE_STATE.DATA : SOURCE_STATE.EMPTY;
        } else {
          const arr = allItems(parsed);
          bundle[name] = arr;
          stepStatus[name] = arr.length ? SOURCE_STATE.DATA : SOURCE_STATE.EMPTY;
        }
      } catch (e) {
        stepFailures += 1;
        stepStatus[name] = SOURCE_STATE.FAILED;
    ...
    // 시간예산 등으로 호출조차 못 한 step → unavailable (persist 는 failed 와 동일 취급 = 기존 값 보존)
    for (const [name] of DETAIL_STEPS) {
      if (!(name in stepStatus)) stepStatus[name] = SOURCE_STATE.UNAVAILABLE;
    }
    ...
    // 저장 필수필드 확인 (listOnly 경로와 동일). 하나라도 없으면 normalizedAll 에 넣지 않는다.
    const missing = ['id', 'external_id', 'name'].filter((k) => norm[k] == null || norm[k] === '');
    if (missing.length) {
      failures.push({ ykiho: maskYkiho(ykiho), step: 'required_fields', reason: 'missing', detail: missing.join(',') });
      continue;
    ...
    // 재수집 판정 해시에 endpoint 상태를 접어 넣는다.
    //   · 필드값만으로는 "endpoint 실패로 값 없음" 과 "권위 있는 빈 값" 이 같은 정규화 결과(예: equipment=null)라
    //     동일 해시가 되어, 앞선 실패로 stale 값이 남은 뒤 진짜 삭제가 skip 될 수 있다.
    //   · sources 를 해시에 포함하면 failed↔success_empty 가 서로 다른 해시가 되어 회복·삭제가 정확히 반영되고,
    //     "동일 부분응답" 재실행은 동일 sources → 동일 해시 → 멱등이 유지된다.
    //   · sources 자체는 해시 입력에만 쓰이고 저장·응답·raw 에는 나가지 않는다.
    norm.normalized_hash = normalizedHash({ core: stripForHash(norm), sources: stepStatus });
    ...
    normalizedAll.push({ hospital: norm, evaluation: evalNorm, raw: rawBundle, sources: stepStatus });
    onProgress({ phase: 'normalized', done: stats.normalized, total: ykihos.length });
  }
  ...
  if (deadlineStop && normalizedAll.length < maxInstitutions) {
    throw new HiraError('수집 시간 예산 초과', {
      op: 'collectHospitals', reason: 'deadline_exceeded', failureKind: 'deadline',
