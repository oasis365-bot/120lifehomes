...
export async function collectHospitals(client, opt = {}) {
  ...
  // client 가 자체 재시도(timeout/network/5xx/code12)를 소진하고 throw 한 "일시적" 오류인가.
  //  data.go.kr 게이트웨이는 수 분 단위로 자가회복하므로(1B-1 실측), client 의 수 초 재시도
  //  위에 collect 레벨에서 몇 번 더(간격 넓게) 시도한다. 'config'(키 미설정)는 재시도 안 함.
  const isTransientThrow = (e) =>
    e && (e instanceof HiraError || e.name === 'HiraError') && e.reason !== 'config';

  // 같은 pageNo 를 최대 LIST_EMPTY_RETRY_MAX 회. 재시도 전 지수 백오프 + jitter.
  //  대상: (1) 정상 resultCode + (범위 내) 0건  (2) client 가 throw 한 일시적 오류.
  //  비대상: 비정상 resultCode 반환(client 가 "비일시적" 으로 판정한 것) — 그대로 abnormal.
  //  endpoint fallback·무한 루프 없음.
  const fetchListPage = async (pn) => {
    ...
    for (let attempt = 1; attempt <= LIST_EMPTY_RETRY_MAX; attempt += 1) {
      ...
      try {
        ...
        thrown = e;
        if (isTransientThrow(e) && attempt < LIST_EMPTY_RETRY_MAX) continue;
    ...
    if (thrown) return { kind: 'error', error: thrown, attempts: LIST_EMPTY_RETRY_MAX };
  ...
  while (ykihos.length < maxInstitutions && pageNo <= 20) {
    ...
    if (r.kind === 'error') {
      warnings.push(`목록 p${pageNo} 조회 실패 (${r.attempts}회) — ${briefErr(r.error)}`);
      if (ykihos.length === 0) {
        // 첫 페이지부터 목록을 못 받음 → 보존할 부분 결과 없음. 빈 배열로 "성공" 반환 안 함.
        throw new HiraError('HIRA 목록 조회 실패', {
          op: 'getHospBasisList',
