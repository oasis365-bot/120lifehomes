...
export async function persistCollected(items, deps = {}) {
  ...
  const status = !anySuccess && stats.failed > 0 ? 'failed' : (stats.failed || stats.partial ? 'partial' : 'ok');
...
/**
 * 이 DB 가 "Preview 전용 빈 DB" 인지 확인. 운영 Supabase 로 보이면 { ok:false }.
 * reason 은 generic 코드만 (URL·행수·키 미노출).
