...
//   · 목록에 없는 필드(hira_ykiho, homepage, medical_services)는 basis/상수 기반이라 endpoint 게이트 없음.
//     - hira_ykiho : 기관 키 (불변)
//     - homepage   : basis(getHospBasisList)의 hospUrl — 기관 처리 자체가 목록 성공이 전제
//     - medical_services : 상수 {} (HIRA 판정 불가). 항상 {} 라 diff 자체가 없음.
 ...
 *   · 값이 바뀐 필드만 대상.
 *   · endpoint 게이트 필드(PROFILE_FIELD_SOURCES)는 그 endpoint 가 이번에 정상 성공했을 때만 반영.
 *     실패/미가용이면 diff 에서 제외 → 기존 DB 값 유지 (부분응답이 데이터를 지우지 않음).
 *   · 정상 성공이면 값이 0/false/빈배열/빈객체여도 그대로 반영 (권위 있는 갱신·삭제).
...
function profileDiff(existing, next, sources) {
  ...
  for (const k of Object.keys(next)) {
    ...
    if (!fieldSourcesOk(k, sources)) continue; // 원천 endpoint 실패/미가용 → 이번엔 건드리지 않음 (기존 값 유지)
    d[k] = next[k];
  }
  return d;
}
