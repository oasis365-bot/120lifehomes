...
// 실 적재(dryRun=false)를 허용할 DB 목적지 hostname.
//  · 코드에 하드코딩하지 않는다 — 환경변수 HOSPITAL_INGEST_DB_HOST 로만 지정.
//  · 미설정이면 fail-closed: 어떤 DB 에도 쓰지 않는다 (verifyPreviewDbUrl → wrong_preview_db).
//    실 적재 시험을 할 때만 해당 배포 스코프(Preview)에 이 변수를 설정한다.
//  · production 은 이 검증 이전에 VERCEL_ENV 로 이미 차단(404 / production_env).
//  · 런타임에 이 값도, SUPABASE_URL 도 응답·에러·로그에 노출하지 않는다 (reason 은 wrong_preview_db 뿐).
export function allowedIngestDbHost(env = {}) {
  const h = typeof env.HOSPITAL_INGEST_DB_HOST === 'string' ? env.HOSPITAL_INGEST_DB_HOST.trim() : '';
  return h || null;
}
...
// hospital_profiles 각 필드가 어느 상세 endpoint(collect 의 step 이름)에서 오는가.
//   · 여기 나열된 step 이 하나라도 정상 성공(success_with_data | success_empty)이 아니면
//     그 필드는 이번 수집에서 갱신하지 않는다 (기존 DB 값 보존).
//   · 목록에 없는 필드는 endpoint 게이트 대상 아님:
//     - hira_ykiho : 기관 키 (불변)
//     - homepage   : basis(getHospBasisList)의 hospUrl — 기관 처리 자체가 목록 성공이 전제
const PROFILE_FIELD_SOURCES = Object.freeze({
  establishment_type: ['facility'],
  bed_total: ['facility'],
  bed_detail: ['facility'],
  specialties: ['departments'],
  specialist_counts: ['departments', 'specialists'],
  equipment: ['equipment'],
});
// 파이프라인이 소유하지 않는 profile 컬럼 — HIRA 로 판정 불가. 값 검증(FACILITY_CLAIMED 등)은
// 운영자/별도 검증 절차의 소유. 수집 파이프라인은 신규 행 생성 시 기본값만 넣고, 이후 절대 갱신하지 않는다.
const PROFILE_NOT_OWNED = new Set(['medical_services']);
// "정상 성공" 상태만 필드 갱신을 허용 (collect.js SOURCE_STATE 의 success_* 두 값).
const SOURCE_USABLE = new Set(['success_with_data', 'success_empty']);
function fieldSourcesOk(field, sources) {
  const req = PROFILE_FIELD_SOURCES[field];
  if (!req) return true; // endpoint 게이트 없는 필드
  if (!sources || typeof sources !== 'object') return false; // 상태 불명 → 보수적으로 보존
  return req.every((step) => SOURCE_USABLE.has(sources[step]));
}
/** 원천 step 중 "정상 성공 + 항목 없음"(권위 있는 빈 결과)이 하나라도 있는가 */
function anyReqStepEmpty(field, sources) {
  const req = PROFILE_FIELD_SOURCES[field];
  if (!req || !sources) return false;
  return req.some((step) => sources[step] === 'success_empty');
}
/** 0·false 는 유효값. null/undefined/''/[]/{} 만 "빈 값". */
function isEmptyProfileValue(v) {
  if (v === null || v === undefined || v === '') return true;
  if (Array.isArray(v)) return v.length === 0;
  if (typeof v === 'object') return Object.keys(v).length === 0;
  return false; // number(0 포함) / boolean(false 포함) / non-empty string
}
/** sources 맵에 실패/미가용 step 이 하나라도 있는가 (ingestion_runs partial 판정용) */
function hasIncompleteSource(sources) {
  if (!sources || typeof sources !== 'object') return false;
  return Object.values(sources).some((st) => !SOURCE_USABLE.has(st));
}
...
export function buildProfileRow(h, facilityId) {
  return {
    ...
    // medical_services: 파이프라인 미소유. 신규 행 생성 시에만 기본값 {} 를 넣고
    //   (컬럼 NOT NULL), 이후 재수집에서는 profileDiff 가 절대 건드리지 않는다
    //   → 운영자/검증 절차가 채운 FACILITY_CLAIMED 등이 보존된다.
    medical_services: {},
  };
}
...
async function processOne(item, ctx) {
  ...
  const sources = item && item.sources; // collect 의 endpoint 상태 (내부 메타 — 저장·응답·raw 미노출)
  ...
  try {
    ...
    // 이번 수집에 실패/미가용 상세 endpoint 가 있었으면 run 을 partial 로 표시 (기관 저장은 계속).
    if (hasIncompleteSource(sources)) stats.sourceIncomplete += 1;
    ...
    if (!prof) {
      try {
        ...
        if (!isConflict(err)) throw err;
        // 경쟁 INSERT — 실제 행을 다시 읽어 source-aware diff 로만 PATCH (덮어쓰기 방지)
        const cur = (await sb(`hospital_profiles?facility_id=eq.${enc(id)}&select=facility_id,${HOSPITAL_PROFILE_FIELDS.join(',')}&limit=1`)).data?.[0] || {};
        const pdiff = profileDiff(cur, profRow, sources);
      ...
      const pdiff = profileDiff(prof, profRow, sources);
      if (Object.keys(pdiff).length) {
        await sb(`hospital_profiles?facility_id=eq.${enc(id)}`, { method: 'PATCH', body: { ...pdiff, updated_at: now() }, prefer: 'return=minimal' });
        wrote = true;
      }
...
/**
 * hospital_profiles PATCH diff — source-aware.
 *   · 값이 바뀐 필드만 대상. facility_id / 파이프라인 미소유 컬럼(medical_services)은 제외.
 *   · endpoint 게이트 필드(PROFILE_FIELD_SOURCES): 그 원천 step 이 하나라도 정상 성공이 아니면
 *     diff 에서 제외 → 기존 DB 값 유지 (endpoint 실패가 데이터를 지우지 않음).
 *   · 원천 step 이 전부 정상 성공이어도:
 *       - 새 값이 present(0·false 포함) → 반영.
 *       - 새 값이 "빈 값"(null/''/[]/{}) → 원천 step 중 하나가 success_empty(권위 있는 "없음")일 때만
 *         반영(삭제 허용). 아니면(success_with_data 인데 optional 원본 키만 누락) → 기존 값 유지.
 * @param {object} existing 현재 DB 행 (없으면 {})
 * @param {object} next     buildProfileRow 결과
 * @param {object} [sources] collect 의 endpoint 상태 맵 { step: 'success_with_data'|... }
 */
function profileDiff(existing, next, sources) {
  const d = {};
  for (const k of Object.keys(next)) {
    if (k === 'facility_id') continue;
    if (PROFILE_NOT_OWNED.has(k)) continue; // 파이프라인 미소유 (medical_services)
    if (canon(existing[k]) === canon(next[k])) continue; // 변경 없음
    if (!fieldSourcesOk(k, sources)) continue; // 원천 endpoint 실패/미가용 → 기존 값 유지
    // 원천 전부 정상 성공. 새 값이 "빈 값"이면 → 원천 step 중 success_empty(권위 있는 "없음")가
    //   하나라도 있을 때만 삭제 반영. 아니면(항목은 왔는데 optional 원본 키만 누락) 기존 값 보존.
    if (existing[k] != null && isEmptyProfileValue(next[k]) && !anyReqStepEmpty(k, sources)) {
      continue;
    }
    d[k] = next[k];
  }
 ...
 * @param {Array<{hospital:object, evaluation:object|null, raw:object, sources?:object}>} items  collect._normalizedAll
 *   item.sources = { <detail step>: 'success_with_data'|'success_empty'|'failed'|'unavailable' }
 *     · 필드별 PATCH 판단용 내부 메타. DB(raw 포함)·HTTP 응답·로그에 그대로 저장/노출하지 않는다.
 *     · 실패/미가용 endpoint 가 담당하는 hospital_profiles 필드는 이번 PATCH 에서 제외 → 기존 값 보존.
 * @param {object} deps  { sb, now }
 * @returns {Promise<{ runId:(number|null), status:string, stats:object, failures:object[] }>}
 */
export async function persistCollected(items, deps = {}) {
  ...
  const stats = {
    ...
    sourceIncomplete: 0, // 상세 endpoint 실패/미가용이 있던 기관 수 (기관 저장은 됨) — run partial 판정
  };
  const failures = [];
  ...
  //  (sourceIncomplete 는 기관별로 new/updated/unchanged 중 하나에 이미 계상되므로 accounted 에 더하지 않는다.)
  ...
  const status =
    !anySuccess && stats.failed > 0 ? 'failed'
      : accounted !== list.length ? 'failed'
        : (stats.failed || stats.partial || stats.sourceIncomplete ? 'partial' : 'ok');
      ...
      await sb(`ingestion_runs?id=eq.${enc(runId)}`, {
        ...
        body: {
          ...
          detail: {
            ...
            sourceIncomplete: stats.sourceIncomplete, // 집계 수치만 (기관 식별정보 없음)
...
/**
 * SUPABASE_URL 이 "정확히" 허용된 DB 목적지를 가리키는지 검증.
 *   · expectedHost 미지정(env HOSPITAL_INGEST_DB_HOST 미설정) → 거부 (fail-closed)
 *   · new URL() 로 파싱 (파싱 불가 → 거부)
 *   · protocol 은 반드시 'https:'
 *   · hostname 은 expectedHost 와 정확 일치 (===). 부분문자열/endsWith/includes 안 씀.
 *   · username / password / port / path(/제외) / query / hash 있으면 거부
 * 실패 시 { ok:false, reason:'wrong_preview_db' } — URL·ref 를 담지 않는다.
 * @param {string} rawUrl
 * @param {string} expectedHost  허용 hostname (보통 allowedIngestDbHost(env))
 * @returns {{ ok:boolean, reason:(string|null) }}
 */
export function verifyPreviewDbUrl(rawUrl, expectedHost) {
  const FAIL = { ok: false, reason: 'wrong_preview_db' };
  const host = typeof expectedHost === 'string' ? expectedHost.trim() : '';
  if (!host) return FAIL; // 허용 목적지 미설정 → 아무 DB 도 통과시키지 않는다
  ...
  if (u.hostname !== host) return FAIL; // 정확 일치만
...
export async function assertPreviewDb(deps = {}) {
  ...
  // (0) DB 목적지 검증 — 어떤 쿼리보다도 먼저. 틀린/미설정 DB 면 접속 자체를 안 한다.
  const dest = verifyPreviewDbUrl(env.SUPABASE_URL, allowedIngestDbHost(env));
